...
(function () {
   ...
   * @api stable
   */
  M.filter.Within = (function (element) {
  ...
  goog.inherits(M.filter.Within, M.Filter);


  /*TODO*/
  M.filter.Within.prototype.execute = function (features) {
    let geometry = this.olParser_.read(this.geometry_);
    let featuresFiltered = features.filter(function (feature) {
      if (geometry.within(this.olParser_.read(feature.getGeometry()))) {
        return feature;
      }
    }.bind(this));
