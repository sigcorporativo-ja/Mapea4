(function (suiteModule) {
   suiteModule.constructorWMS = function () {
      suite('Constructor con parámetro \'wms\'', function () {
         var layer;
         ...
         test('Falla porque intento añadir una capa que de tipo desconocido (1)', function () {
            assert.throw(function () {
               var mapa = M.map({
                  container: 'constructor-tests',
                  wms: 'tipoInexistente'.concat('*').concat(TITLE_WMS)
         ...
         test('Falla porque intento añadir una capa que de tipo que no es WMS (1)', function () {
            assert.throw(function () {
               var mapa = M.map({
                  container: 'constructor-tests',
                  wms: 'KML'.concat('*').concat(TITLE_WMS)
                     .concat('*').concat(URL_WMS).concat('*').concat(NAME_WMS)
                     .concat('*').concat(TRANSPARENCE_WMS).concat('*').concat(TILED_WMS)
               });
            });
         });
         test('Falla porque intento añadir una capa que de tipo que no es WMS (2)', function () {
            assert.throw(function () {
               var mapa = M.map({
                  container: 'constructor-tests',
                  wms: {
