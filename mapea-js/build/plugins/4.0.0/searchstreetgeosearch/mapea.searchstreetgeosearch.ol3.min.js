...
this.search_(b)},this),this.delayTime_))};M.plugin.Autocomplete.prototype.search_=function(a,b){var c=this;this.evt=b;var d;d=M.utils.addParameters(this.url_,{input:a.trim(),limit:this.limit_,codine:this.locality_});d=this.formatContent_(d," ","%20");this.searchTime_=Date.now();(function(b){M.remote.get(d).then(function(d){if(b===c.searchTime_){d=JSON.parse(d.text);d=c.parseResultsForTemplate_(d);var g=c.map_.getControls();if(M.utils.isUndefined(d.docs)){d=0;for(var h=g.length;d<h;d++)"searchstreet"===
g[d].name_?g[d].completed=!1:"searchstreetgeosearch"===g[d].name_&&(g[d].ctrlSearchstreet.completed=!1)}else if(M.utils.isUndefined(d.docs[0]))for(d=0,h=g.length;d<h;d++)"searchstreet"===g[d].name_?g[d].completed=!1:"searchstreetgeosearch"===g[d].name_&&(g[d].ctrlSearchstreet.completed=!1);else M.template.compile(M.plugin.Autocomplete.RESULTAUTOCOMPLETE,{jsonp:!0,vars:d}).then(function(b){c.resultsContainer_.innerHTML=b.innerHTML;c.addEvents_(c.resultsContainer_.querySelectorAll("div.autocomplete"));
if(!0===c.busqMunicipio_)if(!0===c.busqMunicipioClick_){if(c.target_.value=a.slice(0,-1),c.resultsContainer_.innerHTML="",!M.utils.isUndefined(c.evt)){b=0;for(var d=g.length;b<d;b++)"searchstreetgeosearch"===g[b].name_?(g[b].ctrlSearchstreet.searchClick_(c.evt),g[b].ctrlSearchstreet.completed=!1,g[b].ctrlGeosearch.searchClick_(c.evt)):"searchstreet"===g[b].name_&&(g[b].searchClick_(c.evt),g[b].completed=!1);c.busqMunicipioClick_=!1}}else{b=c.resultsContainer_.querySelectorAll("div.autocomplete");
for(var d=0,e=g.length;d<e;d++)if("searchstreetgeosearch"===g[d].name_)if(!0===g[d].ctrlSearchstreet.completed)g[d].ctrlSearchstreet.completed=!1,c.resultsContainer_.innerHTML="";else for(var f=0,h=b.length;f<h;f++)c.evtClickMunicipaly_(b[f]);else if("searchstreet"===g[d].name_)if(!0===g[d].completed)g[d].completed=!1,c.resultsContainer_.innerHTML="";else for(f=0,h=b.length;f<h;f++)c.evtClickMunicipaly_(b[f])}else for(b=0,d=g.length;b<d;b++)"searchstreet"===g[b].name_?!0===g[b].completed&&(c.resultsContainer_.innerHTML=
"",g[b].completed=!1):"searchstreetgeosearch"===g[b].name_&&!0===g[b].ctrlSearchstreet.completed&&(c.resultsContainer_.innerHTML="",g[b].ctrlSearchstreet.completed=!1)});goog.dom.classlist.remove(c.resultsContainer_,M.plugin.Autocomplete.MINIMUM);goog.dom.removeChildren(c.resultsContainer_,c.searchingResult_)}})})(this.searchTime_)};M.plugin.Autocomplete.prototype.evtClickMunicipaly_=function(a){var b=this;goog.events.listen(a,goog.events.EventType.CLICK,function(a){b.clickMunicipaly_()},!1,this)};
...
M.control.Geosearch.SEARCHING_CLASS);var c=M.utils.addParameters(this.searchUrl_,{q:a,start:this.results_.length,srs:this.facadeMap_.getProjection().code}),d=this;(function(a){M.remote.get(c).then(function(c){if(a===d.searchTime_){var g;try{g=JSON.parse(c.text)}catch(h){M.exception("La respuesta no es un JSON v\u00e1lido: "+h)}b.call(d,g);goog.dom.classlist.remove(d.element_,M.control.Geosearch.SEARCHING_CLASS)}})})(this.searchTime_)};M.control.Geosearch.prototype.showResults_=function(a){this.getImpl().clear();
this.drawResults(a);var b=this.parseResultsForTemplate_(a),c=this;M.template.compile(M.control.Geosearch.RESULTS_TEMPLATE,{jsonp:!0,vars:b}).then(function(b){goog.dom.classlist.remove(c.resultsContainer_,M.control.Geosearch.HIDDEN_RESULTS_CLASS);M.utils.isNullOrEmpty(c.resultsScrollContainer_)||goog.events.unlisten(c.resultsScrollContainer_,goog.events.EventType.SCROLL,c.resultsScroll_,!1,c);for(var e=c.resultsContainer_.querySelectorAll(".result"),f,g=0,h=e.length;g<h;g++)f=e.item(g),goog.events.unlisten(f,
goog.events.EventType.CLICK,c.resultClick_,!1,c);0<a.response.docs.length&&c.zoomToResults();e=c.resultsContainer_.querySelector("div.page > div.g-cartografia-flecha-arriba");M.utils.isNullOrEmpty(e)||goog.events.unlisten(e,goog.events.EventType.CLICK,c.resultsClick_,!1,c);c.resultsContainer_.innerHTML=b.innerHTML;c.resultsScrollContainer_=c.resultsContainer_.querySelector("div#m-geosearch-results-scroll");M.utils.enableTouchScroll(c.resultsScrollContainer_);goog.events.listen(c.resultsScrollContainer_,
goog.events.EventType.SCROLL,c.resultsScroll_,!1,c);e=c.resultsContainer_.getElementsByClassName("result");g=0;for(h=e.length;g<h;g++)f=e.item(g),goog.events.listen(f,goog.events.EventType.CLICK,c.resultClick_,!1,c);e=c.resultsContainer_.querySelector("div.page > div.g-cartografia-flecha-arriba");goog.events.listen(e,goog.events.EventType.CLICK,c.resultsClick_,!1,c);c.checkScrollSearch_(a);c.fire(M.evt.COMPLETED)})};M.control.Geosearch.prototype.drawResults=function(a){this.getImpl().drawResults(a)};
...
a instanceof M.control.Geosearch&&(b=this.name===a.name);return b};M.control.Geosearch.prototype.parseResultsForTemplate_=function(a,b){var c="";M.utils.isNullOrEmpty(this.input_)||(c=this.input_.value);var d=0;M.utils.isUndefined(a.spatial_response)?(d=a.response.numFound,this.spatialSearch_=!1):(d=a.spatial_response.numFound,this.spatialSearch_=!0);var e=this.getDocsFromResults_(a);this.results_=!0===b?this.results_.concat(e):e;var f=this.spatialSearch_&&M.utils.isNullOrEmpty(a.spatial_response.docs),
g=null;return g=0!==d?{docs:e,total:d,partial:f}:{docs:e,total:d,notResutls:!0,query:c}};M.control.Geosearch.prototype.getDocsFromResults_=function(a){var b=[],b=this.spatialSearch_?a.spatial_response.docs:a.response.docs,c=["geom","_version_","keywords","solrid"];return b=b.map(function(a){var b=[],f;for(f in a)M.utils.includes(c,f)||b.push({key:M.utils.beautifyAttributeName(f),value:a[f]});return{solrid:a.solrid,attributes:b}})};M.control.Geosearch.prototype.checkScrollSearch_=function(a){var b=
0,b=M.utils.isUndefined(a.spatial_response)?a.response.numFound:a.spatial_response.numFound;this.results_.length!==b||M.utils.isNullOrEmpty(this.resultsScrollContainer_)||goog.events.unlisten(this.resultsScrollContainer_,goog.events.EventType.SCROLL,this.resultsScroll_,!1,this)};M.control.Geosearch.NAME="geosearch";M.control.Geosearch.TEMPLATE="geosearch.html";M.control.Geosearch.RESULTS_TEMPLATE="geosearchresults.html";M.control.Geosearch.HELP_TEMPLATE="geosearchhelp.html";M.control.Geosearch.SEARCHING_CLASS=
"m-searching";M.control.Geosearch.HIDDEN_RESULTS_CLASS="hidden"})();P.impl={};P.impl.utils={};P.impl.utils.Geosearch={};
...
a.name);return b};M.impl.layer.Geosearch.setStyleFeature_=function(a,b){M.impl.geosearch.style.init();M.utils.isArray(a)||(a=[a]);a.forEach(function(a){var d=a.getGeometry().getType();M.utils.isNullOrEmpty(b)||b===M.style.state.DEFAULT?a.setStyle(M.impl.geosearch.style.DEFAULT[d]):b===M.style.state.NEW?a.setStyle(M.impl.geosearch.style.NEW[d]):b===M.style.state.SELECTED&&a.setStyle(M.impl.geosearch.style.SELECTED[d])})};M.impl.layer.Geosearch.POPUP_RESULT="geosearchfeaturepopup.html"})();goog.dom.classes={};goog.dom.classes.set=function(a,b){a.className=b};goog.dom.classes.get=function(a){a=a.className;return goog.isString(a)&&a.match(/\S+/g)||[]};goog.dom.classes.add=function(a,b){var c=goog.dom.classes.get(a),d=goog.array.slice(arguments,1),e=c.length+d.length;goog.dom.classes.add_(c,d);goog.dom.classes.set(a,c.join(" "));return c.length==e};
...
(function(){M.control.Searchstreet=function(a,b,c){M.utils.isUndefined(M.impl.control.Searchstreet)&&M.exception("La implementaci\u00f3n usada no puede crear controles Searchstreet");this.resultsContainer_=this.element_=this.button_=this.input_=null;this.searchTime_=0;this.name_="searchstreet";this.searchUrl_=a;this.provincia_=this.municipio_=null;this.provincias_="huelva sevilla c\u00f3rdoba ja\u00e9n c\u00e1diz m\u00e1laga granada almer\u00eda".split(" ");M.utils.isUndefined(b)||(this.codIne_=b,
this.searchCodIne_=M.config.SEARCHSTREET_URLCOMPROBARINE);this.minAutocomplete_=M.config.AUTOCOMPLETE_MINLENGTH;this.resultsAutocomplete_=this.facadeMap_=null;this.completed=!1;this.respuestasProvincias_=[];this.contadorProvincias=0;this.resultsScrollContainer_=this.resultsContainer_=null;a=new M.impl.control.Searchstreet;M.Control.call(this,a)};goog.inherits(M.control.Searchstreet,M.Control);M.control.Searchstreet.prototype.createView=function(a){var b=this;this.facadeMap_=a;return new Promise(function(a,
d){M.template.compile(M.control.Searchstreet.TEMPLATE,{jsonp:!0}).then(function(d){b.addEvents(d);a(d)})})};M.control.Searchstreet.prototype.addEvents=function(a){var b=this;this.element_=a;this.on(M.evt.COMPLETED,function(){goog.dom.classlist.add(this.element_,"shown")},this);this.input_=this.element_.getElementsByTagName("input")["m-searchstreet-search-input"];this.button_=this.element_.getElementsByTagName("button")["m-searchstreet-search-btn"];this.clear_=this.element_.getElementsByTagName("button")["m-searchstreet-clear-btn"];
goog.events.listen(this.input_,goog.events.EventType.KEYUP,this.searchClick_,!1,this);goog.events.listen(this.button_,goog.events.EventType.CLICK,this.searchClick_,!1,this);goog.events.listen(this.clear_,goog.events.EventType.CLICK,this.clearSearchs_,!1,this);this.resultsContainer_=this.element_.getElementsByTagName("div")["m-searchstreet-results"];this.resultsAutocomplete_=this.element_.getElementsByTagName("div")["m-autocomplete-results"];this.searchingResult_=this.element_.querySelector("div#m-searchstreet-results > div#m-searching-result-searchstreet");
M.utils.isUndefined(this.codIne_)||M.utils.isNullOrEmpty(this.codIne_)||(a=M.utils.addParameters(this.searchCodIne_,{codigo:this.codIne_}),M.remote.get(a).then(function(a){var d;try{M.utils.isNullOrEmpty(a.text)||(d=JSON.parse(a.text),M.utils.isNullOrEmpty(d.comprobarCodIneResponse.comprobarCodIneReturn)?M.dialog.error("El c\u00f3digo del municipio '"+b.codIne_+"' no es v\u00e1lido"):(b.getMunProv_(d),b.element_.getElementsByTagName("span").codIne.innerHTML="B\u00fasquedas en "+b.municipio_+"  ("+
...
this.showResults_);else{var b=a.indexOf(",");-1<a.indexOf(",")&&(a=a.substring(0,b));this.search_(a+", "+this.municipio_+" ("+this.provincia_+")",this.showResults_)}};M.control.Searchstreet.prototype.search_=function(a,b){var c=this,d=null;this.municipio_=this.provincia_=null;goog.dom.appendChild(this.resultsContainer_,this.searchingResult_);goog.dom.classlist.add(this.element_,M.control.Searchstreet.SEARCHING_CLASS);goog.dom.classlist.add(this.resultsContainer_,M.control.Searchstreet.MINIMUM);var e=
M.utils.addParameters(M.config.SEARCHSTREET_NORMALIZAR,{cadena:a});M.remote.get(e).then(function(a){a=JSON.parse(a.text).normalizarResponse.normalizarReturn;c.provincia_=M.utils.beautifyString(a.provincia);c.municipio_=M.utils.beautifyString(a.municipio);if(M.utils.isNullOrEmpty(c.provincia_))if(M.utils.isNullOrEmpty(c.provincia_)&&!M.utils.isNullOrEmpty(c.municipio_)){c.searchTime_=Date.now();c.respuestasProvincias_=[];for(var e=c.contadorProvincias=0,h=c.provincias_.length;e<h;e++)d=M.utils.addParameters(c.searchUrl_,
...
streetNumber:a.numeroPortal,streetType:a.tipoVia,municipio:c.municipio_,provincia:c.provincia_,srs:c.facadeMap_.getProjection().code}),c.searchTime_=Date.now(),c.querySearch_(d,c.provincia_,b)})};M.control.Searchstreet.prototype.querySearch_=function(a,b,c){var d=this;(function(e){M.remote.get(a).then(function(a){if(e===d.searchTime_){var g;try{g=M.utils.isNullOrEmpty(a.text)||-1!=a.text.indexOf("No se ha podido obtener el codigoINE")?null:JSON.parse(a.text)}catch(h){M.exception("La respuesta no es un JSON v\u00e1lido: "+
h)}M.utils.isNullOrEmpty(g)||(d.provincia_=M.utils.beautifyString(b));c.call(d,g);goog.dom.classlist.remove(d.element_,M.control.Searchstreet.SEARCHING_CLASS);goog.dom.classlist.remove(d.resultsContainer_,M.control.Searchstreet.MINIMUM)}})})(this.searchTime_)};M.control.Searchstreet.prototype.querySearchProvinces=function(a,b,c){var d=this;(function(e){M.remote.get(a).then(function(a){d.respuestasProvincias_.push(a);d.contadorProvincias++;if(8==d.contadorProvincias){a=0;for(var g=d.respuestasProvincias_.length;a<
g;a++)if(e===d.searchTime_){var h;try{var k=d.respuestasProvincias_[a].text;h=M.utils.isNullOrEmpty(k)||-1!=k.indexOf("No se ha podido obtener el codigoINE")?null:JSON.parse(k)}catch(l){M.exception("La respuesta no es un JSON v\u00e1lido: "+l)}M.utils.isNullOrEmpty(h)||0===h.geocoderMunProvSrsResponse.geocoderMunProvSrsReturn.geocoderMunProvSrsReturn.coordinateX||(d.provincia_=M.utils.beautifyString(b),c.call(d,h),goog.dom.classlist.remove(d.element_,M.control.Searchstreet.SEARCHING_CLASS),goog.dom.classlist.remove(d.resultsContainer_,
...
M.control.Searchstreet.MINIMUM);a=b.resultsContainer_.querySelector("div.page > div.g-cartografia-flecha-arriba");goog.events.listen(a,goog.events.EventType.CLICK,b.resultsClick_,!1,b);b.fire(M.evt.COMPLETED)});this.element_.getElementsByTagName("div")["m-autocomplete-results"].innerHTML=""};M.control.Searchstreet.prototype.parseResultsForTemplate_=function(a){var b=null,b=null,c=this.input_.value;if(M.utils.isNullOrEmpty(a))b={docs:[void 0],containtResult:b,search:c};else if(a=a.geocoderMunProvSrsResponse.geocoderMunProvSrsReturn,
...
function(a){this.drawNewResults(a);var b=this.parseResultsForTemplate_(a,!0),c=this;M.template.compile(M.control.Geosearch.RESULTS_TEMPLATE,{jsonp:!0,vars:b}).then(function(b){b=b.getElementsByTagName("div")["m-geosearch-results-scroll"].children;for(var e;null!==(e=b.item(0));)c.resultsScrollContainer_.appendChild(e),goog.events.listen(e,goog.events.EventType.CLICK,c.resultClick_,!1,c);c.resultsContainer_.getElementsByTagName("span")["m-geosearch-page-found"].innerHTML=c.results_.length;goog.dom.classlist.remove(c.element_,
...
(function(){M.plugin.SearchstreetGeosearch=function(a){a=a||{};this.name="searchstreetgeosearch";this.parameters_=a;this.control_=this.map_=null;this.urlAutocomplete_=a.urlAutocomplete;this.minLengthAutocomplete_=a.minLengthAutocomplete;this.delayAutocomplete_=a.delayAutocomplete;this.limitAutocomplete_=a.limitAutocomplete;this.locality_=a.locality;M.Plugin.call(this)};goog.inherits(M.plugin.SearchstreetGeosearch,M.Plugin);M.plugin.SearchstreetGeosearch.prototype.addTo=function(a){this.map_=a;var b=
this;this.control_=new M.control.SearchstreetGeosearch(this.parameters_);goog.dom.classlist.add(a._areasContainer.getElementsByClassName("m-top m-right")[0],"top-extra");this.control_.on(M.evt.ADDED_TO_MAP,function(){this.fire(M.evt.ADDED_TO_MAP);var a=M.utils.addParameters(M.config.SEARCHSTREET_URLCOMPROBARINE,{codigo:this.locality_});M.remote.get(a).then(function(a){a=JSON.parse(a.text);M.utils.isNullOrEmpty(a.comprobarCodIneResponse.comprobarCodIneReturn)&&(b.locality_="");a=new M.plugin.Autocomplete({locality:b.locality_,
url:b.urlAutocomplete_,minLength:b.minLengthAutocomplete_,delay:b.delayAutocomplete_,target:b.control_.getInput(),limit:b.limitAutocomplete_,html:b.control_.getHtml()});b.map_.addPlugin(a)})},this);this.panel_=new M.ui.Panel("SearchstreetGeosearch",{collapsible:!0,className:"m-geosearch",position:M.ui.position.TL});this.panel_.addControls(this.control_);this.map_.addPanels(this.panel_)};M.plugin.SearchstreetGeosearch.prototype.destroy=function(){this.map_.removeControls([this.control_]);this.locality_=
this.limitAutocomplete_=this.delayAutocomplete_=this.minLengthAutocomplete_=this.urlAutocomplete_=this.control_=this.map_=this.parameters_=this.name=null}})();P.impl.control.SearchstreetIntegrated={};(function(){M.impl.control.SearchstreetIntegrated=function(){M.impl.control.Searchstreet.call(this)};goog.inherits(M.impl.control.SearchstreetIntegrated,M.impl.control.Searchstreet);M.impl.control.SearchstreetIntegrated.prototype.addTo=function(a,b){this.facadeMap_=a;this.element_=b;ol.control.Control.call(this,{element:b,target:null})};M.impl.control.SearchstreetIntegrated.prototype.zoomResults=function(){}})();P.control.SearchstreetIntegrated={};
...
(function(){M.control.SearchstreetGeosearch=function(a){M.utils.isUndefined(M.impl.control.SearchstreetGeosearch)&&M.exception("La implementaci\u00f3n usada no puede crear controles SearchStreetGeosearch");a=a||{};this.urlSearchstret_=M.config.SEARCHSTREET_URL;M.utils.isNullOrEmpty(a.urlSearchstret)||(this.urlSearchstret_=a.urlSearchstret);M.utils.isNullOrEmpty(a.locality)||(this.locality_=a.locality);this.urlGeosearch_=M.config.GEOSEARCH_URL;M.utils.isNullOrEmpty(a.urlGeosearch)||(this.urlGeosearch_=
a.urlGeosearch);this.coreGeosearch_=M.config.GEOSEARCH_CORE;M.utils.isNullOrEmpty(a.coreGeosearch)||(this.coreGeosearch_=a.coreGeosearch);this.handlerGeosearch_=M.config.GEOSEARCH_HANDLER;M.utils.isNullOrEmpty(a.handlerGeosearch)||(this.handlerGeosearch_=a.handlerGeosearch);this.paramsGeosearch_=a.paramsGeosearch||{};this.ctrlGeosearch=this.ctrlSearchstreet=this.input_=null;this.name_="searchstreetgeosearch";this.facadeMap_=this.element_=null;a=new M.impl.control.SearchstreetGeosearch;M.Control.call(this,
a,this.name_)};goog.inherits(M.control.SearchstreetGeosearch,M.Control);M.control.SearchstreetGeosearch.prototype.addTo=function(a){var b=this,c=this.getImpl();this.facadeMap_=a;var d=this.createView(a);d instanceof Promise&&d.then(function(d){c.addTo(a,d);b.html=d;b.fire(M.evt.ADDED_TO_MAP)});this.on(M.evt.ADDED_TO_MAP,function(c){M.utils.isUndefined(b.locality_)?b.ctrlSearchstreet=new M.control.SearchstreetIntegrated(b.urlSearchstret_):b.ctrlSearchstreet=new M.control.SearchstreetIntegrated(b.urlSearchstret_,
b.locality_);c=b.ctrlSearchstreet.getImpl();b.ctrlSearchstreet.createView(b.html,a);c.addTo(a,b.html);b.ctrlGeosearch=new M.control.GeosearchIntegrated(b.urlGeosearch_,b.coreGeosearch_,b.handlerGeosearch_,b.paramsGeosearch_);c=b.ctrlGeosearch.getImpl();b.ctrlGeosearch.createView(b.html,b.facadeMap_);c.addTo(a,b.html);var d=!1;b.ctrlSearchstreet.on(M.evt.COMPLETED,function(){!0===d&&(goog.dom.classlist.add(b.element_,"shown"),d=!1,b.zoomResults());d=!0},this);b.ctrlGeosearch.on(M.evt.COMPLETED,function(){!0===
d&&(goog.dom.classlist.add(b.element_,"shown"),d=!1,b.zoomResults());d=!0},this)})};M.control.SearchstreetGeosearch.prototype.createView=function(a){var b=this;return new Promise(function(a,d){M.template.compile(M.control.SearchstreetGeosearch.TEMPLATE,{jsonp:!0}).then(function(d){b.element_=d;b.input_=d.querySelector("input#m-searchstreetgeosearch-search-input");var f=d.querySelector("div#m-searchstreet-results"),g=d.querySelector("div#m-geosearch-results"),h=d.querySelector("ul#m-tabs > li:nth-child(1) > a"),
k=d.querySelector("ul#m-tabs > li:nth-child(2) > a");goog.events.listen(h,goog.events.EventType.CLICK,function(a){a.preventDefault();goog.dom.classlist.contains(h,"activated")||(goog.dom.classlist.add(h,"activated"),goog.dom.classlist.remove(k,"activated"),goog.dom.classlist.add(f,"show"),goog.dom.classlist.remove(g,"show"))},!1,this);goog.events.listen(k,goog.events.EventType.CLICK,function(a){a.preventDefault();goog.dom.classlist.contains(k,"activated")||(goog.dom.classlist.add(k,"activated"),goog.dom.classlist.remove(h,
"activated"),goog.dom.classlist.remove(f,"show"),goog.dom.classlist.add(g,"show"))},!1,this);a(d)})})};M.control.SearchstreetGeosearch.prototype.getInput=function(){return this.input_};M.control.SearchstreetGeosearch.prototype.equals=function(a){var b=!1;a instanceof M.control.SearchstreetGeosearch&&(b=this.name===a.name);return b};M.control.SearchstreetGeosearch.prototype.getHtml=function(){return this.element_};M.control.SearchstreetGeosearch.prototype.zoomResults=function(){this.getImpl().zoomResults()};
M.control.SearchstreetGeosearch.TEMPLATE="searchstreetgeosearch.html"})();P.impl.control.SearchstreetGeosearch={};
...
goog.exportProperty(M.control.SearchstreetGeosearch.prototype,"createView",M.control.SearchstreetGeosearch.prototype.createView);goog.exportProperty(M.control.SearchstreetGeosearch.prototype,"getInput",M.control.SearchstreetGeosearch.prototype.getInput);goog.exportProperty(M.control.SearchstreetGeosearch.prototype,"equals",M.control.SearchstreetGeosearch.prototype.equals);goog.exportSymbol("M.control.SearchstreetGeosearch.TEMPLATE",M.control.SearchstreetGeosearch.TEMPLATE);
goog.exportSymbol("M.impl.control.SearchstreetIntegrated",M.impl.control.SearchstreetIntegrated);goog.exportProperty(M.impl.control.SearchstreetIntegrated.prototype,"addTo",M.impl.control.SearchstreetIntegrated.prototype.addTo);goog.exportProperty(M.impl.control.SearchstreetIntegrated.prototype,"zoomResults",M.impl.control.SearchstreetIntegrated.prototype.zoomResults);goog.exportSymbol("M.control.SearchstreetIntegrated",M.control.SearchstreetIntegrated);
goog.exportProperty(M.control.SearchstreetIntegrated.prototype,"createView",M.control.SearchstreetIntegrated.prototype.createView);goog.exportProperty(M.control.SearchstreetIntegrated.prototype,"addEvents",M.control.SearchstreetIntegrated.prototype.addEvents);goog.exportProperty(M.control.SearchstreetIntegrated.prototype,"getImpl",M.control.SearchstreetIntegrated.prototype.getImpl); })(window.M);
