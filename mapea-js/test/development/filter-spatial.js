...
const miFeature = new Feature('featurePrueba002', {
  type: 'Feature',
...
function filtrar() {
  const filtroCapa = lyProvincias.getFilter();
  // eslint-disable-next-line
  const nProv = prompt('Introduzca parte del nombre de la provincia', 'Sevilla');
  let filter = LIKE('nombre', nProv);
  if (!isNullOrEmpty(filtroCapa)) {
    if (document.querySelector('#chkY').checked) {
